// src/lib/clubArticles.ts

// We keep this file as the single source of truth for "Club Articles".
// Admin dropdown options + backend validation + homepage insertion all use this.

export const CLUB_ARTICLES_REQUIRED_COUNT = 5 as const;

/**
 * Where the Club Articles block can be injected on the homepage.
 * Keep these keys stable (they become stored in DB later).
 */
export const CLUB_POSITIONS = [
  "after_top_stories",

  "after_world_section",
  "after_india_section",
  "after_business_section",
  "after_technology_section",
  "after_entertainment_section",
  "after_sports_section",
  "after_health_section",
  "after_lifestyle_section",
] as const;

export type ClubPosition = (typeof CLUB_POSITIONS)[number];

/**
 * Labels shown in admin dropdown.
 * (Keys must match CLUB_POSITIONS exactly.)
 */
export const CLUB_POSITION_LABELS: Record<ClubPosition, string> = {
  after_top_stories: "After Top Stories",

  after_world_section: "After World Section",
  after_india_section: "After India Section",
  after_business_section: "After Business Section",
  after_technology_section: "After Technology Section",
  after_entertainment_section: "After Entertainment Section",
  after_sports_section: "After Sports Section",
  after_health_section: "After Health Section",
  after_lifestyle_section: "After Lifestyle Section",
};

/**
 * Stored config structure (DB will store this later).
 */
export type ClubArticlesConfig = {
  position: ClubPosition;
  articleIds: string[]; // exactly 5 ids, ordered
  updatedAt: string; // ISO string
};

/**
 * Payload shape from Admin -> API (we will use this in Step 3).
 */
export type ClubArticlesPayload = {
  position: ClubPosition;
  articleIds: string[];
};

export function isClubPosition(value: unknown): value is ClubPosition {
  return typeof value === "string" && (CLUB_POSITIONS as readonly string[]).includes(value);
}

/**
 * Returns normalized IDs: trimmed, non-empty, unique (preserves order)
 */
export function normalizeArticleIds(ids: unknown): string[] {
  if (!Array.isArray(ids)) return [];
  const seen = new Set<string>();
  const out: string[] = [];

  for (const raw of ids) {
    if (typeof raw !== "string") continue;
    const id = raw.trim();
    if (!id) continue;
    if (seen.has(id)) continue;
    seen.add(id);
    out.push(id);
  }

  return out;
}

/**
 * Strict validation for club creation/update.
 * - position must be allowed
 * - articleIds must be exactly 5 unique ids
 */
export function validateClubArticlesPayload(input: unknown): {
  ok: true;
  data: ClubArticlesPayload;
} | {
  ok: false;
  error: string;
} {
  if (!input || typeof input !== "object") {
    return { ok: false, error: "Invalid payload" };
  }

  const obj = input as Record<string, unknown>;

  if (!isClubPosition(obj.position)) {
    return { ok: false, error: "Invalid position" };
  }

  const articleIds = normalizeArticleIds(obj.articleIds);

  if (articleIds.length !== CLUB_ARTICLES_REQUIRED_COUNT) {
    return {
      ok: false,
      error: `Select exactly ${CLUB_ARTICLES_REQUIRED_COUNT} articles`,
    };
  }

  return {
    ok: true,
    data: {
      position: obj.position,
      articleIds,
    },
  };
}
