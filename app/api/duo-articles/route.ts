// app/api/duo-articles/route.ts
export const dynamic = "force-dynamic";

import { NextResponse } from "next/server";
import { getDb } from "@/src/lib/db";
import {
  validateDuoArticlesPayload,
  type DuoArticlesConfig,
...
export async function GET() {
  ...
  const config: DuoArticlesConfig = {
...
export async function POST(req: Request) {
  ...
  const validation = validateDuoArticlesPayload(body);
  if (!validation.ok) {
  ...
  db.prepare(
    `
    INSERT INTO duo_articles (id, position, articleIds, updatedAt)
    VALUES (1, ?, ?, ?)
...
export async function DELETE() {
  const db = getDb();
  db.prepare(`DELETE FROM duo_articles WHERE id = 1`).run();
