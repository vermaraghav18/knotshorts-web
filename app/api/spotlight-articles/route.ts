// app/api/spotlight-articles/route.ts
export const dynamic = "force-dynamic";

import { NextResponse } from "next/server";
import { getDb } from "@/src/lib/db";
import {
  validateSpotlightArticlesPayload,
  type SpotlightArticlesConfig,
...
export async function GET() {
  ...
  const config: SpotlightArticlesConfig = {
...
export async function POST(req: Request) {
  ...
  const validation = validateSpotlightArticlesPayload(body);
  if (!validation.ok) {
  ...
  db.prepare(
    `
    INSERT INTO spotlight_articles (id, position, articleIds, updatedAt)
    VALUES (1, ?, ?, ?)
...
export async function DELETE() {
  const db = getDb();
  db.prepare(`DELETE FROM spotlight_articles WHERE id = 1`).run();
